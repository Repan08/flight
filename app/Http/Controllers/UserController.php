...
class UserController extends Controller
{
    ...
    {
        // Ambil booking milik user saat ini (jika ada)
        $user = Auth::user();
        $bookings = [];
        $stats = [
            'total' => 0,
        ...
        if ($user) {
            $bookings = \App\Models\Booking::with(['flight', 'payment'])
                ->where('user_id', $user->id)
                ->orderBy('created_at', 'desc')
                ->get();

            $stats['total'] = $bookings->count();
            $stats['confirmed'] = $bookings->where('status', 'confirmed')->count();
            $stats['pending'] = $bookings->where('status', 'pending')->count();
            $stats['spent'] = $bookings->sum('total_price');
        }
    ...
    public function showConfirmPayment($id)
    {
        ...
        return view('user.confirm-payment', compact('booking'));
    }

    public function getPaymentProof($fileName)
    {
        $filePath = storage_path('app/public/payments/' . $fileName);
